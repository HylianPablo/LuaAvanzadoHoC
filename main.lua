...
function love.update(dt)
    ...
    love.audio.play(music)  --Música de fondo.
    ...
    for i, asteroid in ipairs(asteroids) do
        if collision(player.x,player.y,player.width,player.height,asteroid.x,asteroid.y,asteroid.width,asteroid.height) then
            life=life-1
            table.remove(asteroids,i)
        end
    end
